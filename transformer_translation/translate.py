...
# BPE Model
bpe_model = youtokentome.BPE(model="/home/savio/Documents/Tutorials/NLP_Tutorials/datasets/wmt-14-eng-deu/bpe.model")
...
def translate(source_sequence, beam_size=4, length_norm_coefficient=0.6):
    ...
    with torch.no_grad():
        ...
        encoder_pad_masks    = (encoder_sequences == bpe_model.subword_to_id('<PAD>')).to(torch.bool).to(device)  # [1, source_sequence_length]

        # Encode
        encoder_sequences = model.encoder(input_sequence=encoder_sequences,
                                          input_padding_mask=encoder_pad_masks)  # (1, source_sequence_length, d_model)
        ...
        while True:
            ...
            decoder_padding_mask = torch.zeros_like(hypotheses).to(torch.bool).to(device)  # [s, step]

            decoder_sequences = model.decoder(input_sequence=hypotheses,
                                              input_padding_mask=decoder_padding_mask,
                                              memory_sequence=encoder_sequences.repeat(s, 1, 1),
                                              memory_padding_mask=encoder_pad_masks.repeat(s, 1))  # (s, step, vocab_size)

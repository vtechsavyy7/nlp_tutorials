...
class SequenceLoader(object):
    ...
    def __next__(self):
        ...
        src_pad_masks    = (source_data == self.bpe_model.subword_to_id('<PAD>')).to(torch.bool)  # [B, src_padded_length]
        target_pad_masks = (target_data == self.bpe_model.subword_to_id('<PAD>')).to(torch.bool)  # [B, target_padded_length]

        # Convert lengths to tensors
        source_lengths = torch.LongTensor(source_lengths)
        target_lengths = torch.LongTensor(target_lengths)

        return source_data, target_data, source_lengths, target_lengths, src_pad_masks, target_pad_masks
